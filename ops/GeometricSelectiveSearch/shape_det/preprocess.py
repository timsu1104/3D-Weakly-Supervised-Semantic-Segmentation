...
data_path = '/home/zhengyuan/code/WyPR/shape_det'
...
def _fix_unassigned_points(line):
    ...
    if os.path.exists(output_f):
        return
        # /home/zhengyuan/code/WyPR/shape_det/cgal_output/scene0403_00_shape.npy
    assert os.path.exists(line)
    with open(line, 'r') as fin:
        cgal_txt = fin.readlines()
    pcd_path = os.path.join(data_path, 'cgal_input/train', scene_id+'.xyzn')
    if not os.path.exists(pcd_path):
        pcd_path = os.path.join(data_path, 'cgal_input/val', scene_id+'.xyzn')
...
def fix_unassigned_points():
    """ assign the un-assigned label to its nearest assigned points """
    anno_paths = glob.glob(os.path.join(data_path, 'cgal_output', '*.txt'))
