...
class CLIPModel:
    ...
    def cond_fn_img(self, images: torch.Tensor, grad_scale: float) -> Callable[..., torch.Tensor]:   
        ...
        print(images.size())
        def cond_fn_img(x, t, grad_scale=grad_scale, **kwargs):
            with torch.no_grad():
                z_i_guide = self.image_embeddings(images, t)
            with torch.enable_grad():
                x_var = x.detach().requires_grad_(True)
                print(x_var.size())
                z_i = self.image_embeddings(x_var, t)
                loss = torch.exp(self.logit_scale) * (z_i_guide * z_i).sum()
                grad = torch.autograd.grad(loss, x_var)[0].detach()
            return grad * grad_scale
